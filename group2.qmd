...
election_data_turnout <- election_data_vote_share |>
  mutate(turnout = (participacion_1 + participacion_2) / 2) |> # Average turnout if needed
  mutate(turnout_bin = ifelse(turnout < median(turnout, na.rm = TRUE), "Low Turnout", "High Turnout"))

party_turnout_summary <- election_data_turnout |>
...
The parties that benefit from low turnout are: CIU, PNV, Bildu, ERC, PP, VOX and Other parties (NA).
...
analyze_low_turnout <- function(data, turnout_col = "turnout", ballots_col = "ballots", censo_col = "censo", party_col = "siglas") {
  ...
  data <- data |>
    mutate(turnout_bin = ifelse(!!sym(turnout_col) < median(!!sym(turnout_col), na.rm = TRUE), 
                                "Low Turnout", 
                                "High Turnout"))
...
turnout_analysis <- analyze_low_turnout(election_data_turnout, 
                                        turnout_col = "turnout", 
                                        ballots_col = "ballots", 
                                        censo_col = "censo", 
                                        party_col = "siglas")
...
OPTION 1: No me deja porque sorpresa, Mac es una mierda, pero os dejo aquí como hacerlo por si alguna puede ejecutarlo. NOTA (Laura): dejo esta opción por si acaso lo que yo he hecho está mal, pero la opción 2 es más corta y mi ordenador puede con ella.
...
election_data_areas <- election_data_tidy_new |> 
  mutate(area_type = case_when(
    censo < 30000 ~ "Rural", #30000 es supuestamente lo que se considera rural
    TRUE ~ "Urban"
  ))  |> 
