...
Mencía: Esta opción me gusta más porq por año se ve más cómo cambia el voto y cómo van apareciendo nuevos partidos y se va acabando con el bipartidismo
...
Mencía: el mapa yo lo reservaría para las preguntas nuestras pq aqui creo q aporta mas información el grafico de barras agrupado por años
...
La mejor opción es la de Laura al final yo creo

OPTION 1: la he quitado pq es imposible (está en el doc copiado)
OPTION 2:
...
ggplot(party_counts, aes(x = reorder(siglas, -municipalities_won), y = municipalities_won, fill = siglas)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("BNG" = "pink", "CS" = "orange",
                    "CIU" = "darkred", "EH-BILDU" = "darkgreen", "ERC" = "yellow", "Other" = "grey", "EAJ-PNV" = "lightblue", "PP" = "blue", "PODEMOS-IU" = "purple", "VOX" = "green", "PSOE" = "red")) +
...
party_counts2 <- low_turnout_winners |> 
  group_by(siglas, anno) |> 
  summarise(municipalities_won = n()) |> 
  ungroup()

library(ggplot2)

ggplot(party_counts2, aes(x = reorder(siglas, -municipalities_won), y = municipalities_won, fill = as.factor(anno))) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("BNG" = "pink", "CS" = "orange",
                    "CIU" = "darkred", "EH-BILDU" = "darkgreen", "ERC" = "yellow", "Other" = "grey", "EAJ-PNV" = "lightblue", "PP" = "blue", "PODEMOS-IU" = "purple", "VOX" = "green", "PSOE" = "red")) +
  labs(title = "Municipalities won by party in areas of low turnout",
...
#Cambio: Low turnout no se basa en la mediana (porque antes daba un numero entero), sino que ahora se basa en un porcentaje 

party_turnout_summary <- turnout_election_data |>
  group_by(siglas, turnout_bin) |>
  summarise(avg_vote_share = mean(vote_share, na.rm = TRUE)) |>
  arrange(desc(avg_vote_share))

ggplot(party_turnout_summary, aes(x = siglas, y = avg_vote_share, fill = turnout_bin)) +
...
We see we get the same results we had with the previous way of doing it.
...
OPTION 1: No me deja porque sorpresa, Mac es una mierda, pero os dejo aquí como hacerlo por si alguna puede ejecutarlo. NOTA (Laura): dejo esta opción por si acaso lo que yo he hecho está mal, pero la opción 2 es más corta y mi ordenador puede con ella. NOTA (Mencía): no he podido verificar el código alba pq se me peta R así q he seguido la opción de Laura q es mas corta y he añadido otro enfoque abajo
...
votes_by_party_exercise_4 <- votes_by_party_all[, c("municipio", "siglas", "anno", "total_votes")]
election_data_exercise_4 <- final_election_data[, c("municipio", "censo", "anno", "siglas", "zone_type")]

votes_with_census <- merge(votes_by_party_exercise_4, election_data_exercise_4, by = c("municipio", "anno", "siglas"))
...
ggplot(votes_with_census, aes(x = censo, y = vote_share, color = siglas)) +
...
```{r}
zone_analysis <- aggregate(vote_share ~ zone_type + siglas, data = votes_with_census, FUN = mean)

zone_analysis
```

Nota (Laura): aquí no entiendo muy bien qué son estos resultados, pero como mi ordenador va a pedales no sé si será cosa de eso. También me salen solo 22 rows y creo que no es normal.

```{r}
# Visualization

ggplot(zone_analysis, aes(x = zone_type, y = vote_share, fill = siglas)) +
...
```{r}
election_data_areas <- election_data_tidy_new |> 
  mutate(area_type = case_when(
    censo < 30000 ~ "Rural",
    TRUE ~ "Urban"
  ))  |> 
  group_by(area_type, party)  |> 
  summarise(total_votes = sum(ballots, na.rm = TRUE))  |> 
  mutate(vote_share = total_votes / sum(total_votes))

#visualization (Mencía: he cambiado colores a los del partido)
party_colors <- c(
...
ggplot(election_data_areas, aes(x = area_type, y = vote_share, fill = party)) +
  geom_bar(stat = "identity") +
  labs(title = "Vote Share by Area Type", x = "Area Type", y = "Vote Share") +
...
MENCÍA:
He hecho otra aproximación de esta pregunta, enfocándolo a: de cada partido cuánto de su porcentaje proviene del voto urbano y cuánto del rural? Es decir, p.e de PSOE cuánto de su electorado es de la zona rural y cuánto es de la zona urbana?
...
```
Si esto está bien aquí podemos ver mejor q por ejemplo en Convergencia i Unio lo votan más del lado rural q del urbano, en cambio en Más País lo votan más en urbano, etc. Debatir q hacemos 

We can also do a linear model to further explore this relationship:

```{r}
lm_model <- lm(ballots ~ censo + party, data = election_data_tidy_new)
summary(lm_model)

```

We see that the census can be a good predictor of ballots cast. Its effects are especially positive in parties like PP or PSOE and negative in parties like Más País.

Nota Mencía: no creo q la Rregresion lineal esté bien del todo (?) o al menos la interpretación y la seleccion de variables no lo tengo claro ... Pq la variable dep cogida es "ballots" y en la interpretación estás hablando de q el efecto del censo (población) sobre los ballots es alto lo q es casi obvio pq con esto vemos que ahí donde hay más población los partidos tienen más votos (+ gente = + votos)

Y luego la interpretación q haces del efecto sobre los partidos no la entiendo, pq los partidos son var indep en el modelo, no dep (entonces no puedes ver efecto sobre partidos, solo sobre ballots). Si realmente queremos ver cómo ser rural o urbano influye en el voto a partidos habría q hacer una regresion multinomial para ver cómo influye rural-urbano sobre el voto a cada partido.  Reservaría hacer una regresion tocha para las preguntas finales y para esto haría un chi2 o ANOVA (pero no sé como sigo pensandolo)
