...
### Original Question 3
**Comparison between the percentage of votes and the allocation of seats by party (2019)**

```{r}
# filter votes for 2019
...
```{r}
# allocate seats (escaños) to parties
...
```{r}
#compare votes proportion and seats proportions
...
Comparing the votes that a party obtains with the seats that it actually obtains in the end, we see that the large parties benefit (PSOE; PP) while the rest (smaller parties) generally lose out because they obtain many fewer seats than votes they obtained (proportionally), like Ciudadanos above all, Podemos-IU, Más País, etc.
The discrepancy between the percentage of votes and seats is due to the D'Hondt electoral system, which favors large parties, since it distributes the seats in such a way that the parties with more votes obtain proportionally more representation. Furthermore, small parties fail to exceed the vote threshold in many constituencies, lose representation in the smallest ones, and have a more dispersed vote, which makes it difficult for them to convert those votes into seats. This means that large parties such as PSOE and PP benefit, while parties such as Ciudadanos or Podemos-IU receive fewer seats than they would proportionally deserve.
...
# calculate Total votes, Blank and Null Votes per Year
...
# calculate percentages of Blank and Null Votes
