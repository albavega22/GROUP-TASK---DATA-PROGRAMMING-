...
#Order in leyend
second_party_year_pp_summary$second_party <- factor(
  second_party_year_pp_summary$second_party,
  levels = c("PP", "PSOE", "PODEMOS-IU", "VOX", "BNG", "CS", "CIU", "EH-BILDU", "ERC", "EAJ-PNV", "MP", "Other")
)

ggplot(second_party_year_psoe_summary, aes(x = factor(anno), y = count, fill = second_party)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("BNG" = "skyblue", "CS" = "#EB6109",
                    "CIU" = "#F7D117", "EH-BILDU" = "lightgreen", "ERC" = "#FFB000", "EAJ-PNV" = "#228B22", "PP" = "#1D84CE", "PODEMOS-IU" = "#6A097D", "VOX" = "#5BC035", "MP"= "#00B1E0", "Other" = "#D3D3D3")) +
...
second_party_year_pp_summary$second_party <- factor(
  second_party_year_pp_summary$second_party,
  levels = c("PSOE", "PP", "PODEMOS-IU", "VOX", "BNG", "CS", "CIU", "EH-BILDU", "ERC", "EAJ-PNV", "MP", "Other")
)

ggplot(second_party_year_pp_summary, aes(x = factor(anno), y = count, fill = second_party)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("BNG" = "skyblue", "CS" = "#EB6109",
                    "CIU" = "#F7D117", "EH-BILDU" = "lightgreen", "ERC" = "#FFB000", "Other" = "#D3D3D3", "EAJ-PNV" = "#228B22", "PP" = "blue", "PODEMOS-IU" = "#6A097D", "VOX" = "#5BC035", "PSOE" = "#FF0000", "MP"= "#00B1E0")) +
...
#Order leyend
vote_share_long$siglas <- factor(
  vote_share_long$siglas,
  levels = c("PP", "PSOE", "PODEMOS-IU", "VOX", "BNG", "CS", "CIU", "EH-BILDU", "ERC", "EAJ-PNV", "MP", "Other")
)
...
### Original Question 2: MAP WITH MOST VOTED PARTIES

Now we will see which parties obtained the most votes in each province in the general elections of each year from 2008 to 2019 (important: the number of votes is not equal to the final result translated into seats)
...
Obtenemos el mapa base de: https://centrodedescargas.cnig.es/CentroDescargas/catalogo.do?Serie=CAANE , concretamente es
...
mapa_con_votos <- mapa_provincias |>
...
#Para el orden de como sale el índice:
mapa_con_votos$siglas <- factor(mapa_con_votos$siglas, levels = c("PP", "PSOE", "PODEMOS-IU", "EAJ-PNV", "Other"))


#Mapa básico (lo he dejado por si empezaba a fallar el mapa más editado abajo. Se puede borrar si no da fallos el de abajo)
...
Esto lo dejo porque es el mapa si lo quisiéramos por municipios:
```{r}
...
Aqui he hecho algo por usar los votos en blancos y nulos pero la vd q es un poco tonteria, se nos puede ocurrir otra cosa para usarlos
```{r}
#| code-fold: true
final_election_data <- final_election_data |>
...
muestra <- final_election_data[sample(nrow(final_election_data), 10000), ]

ggplot(muestra, aes(x = participacion, y = votos_blancos)) +
  geom_point(color="purple") +  
  geom_smooth(method = "lm", se = FALSE, color = "black") +  
  labs(
    title = "Relación entre participación y votos blancos",
    x = "Participación (%)",
    y = "Votos Blancos",
